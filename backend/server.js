...
// ── Silence EPIPE on destroyed sockets before it ever reaches Express ──
// Wraps res.write / res.end so a disconnected client never throws.
const _resWrite = http.ServerResponse.prototype.write;
const _resEnd   = http.ServerResponse.prototype.end;
http.ServerResponse.prototype.write = function(chunk, encoding, cb) {
  if (this.socket && this.socket.destroyed) return false;
  try { return _resWrite.call(this, chunk, encoding, cb); }
  catch (e) { if (e.code !== 'EPIPE' && e.code !== 'ECONNRESET') throw e; return false; }
};
http.ServerResponse.prototype.end = function(chunk, encoding, cb) {
  if (this.socket && this.socket.destroyed) return this;
  try { return _resEnd.call(this, chunk, encoding, cb); }
  catch (e) { if (e.code !== 'EPIPE' && e.code !== 'ECONNRESET') throw e; return this; }
};
...
async function sendFcmPushForRequirement(requirement) {
  ...
  const { bloodType, patientName, hospital, urgency, unitsRequired, _id, createdBy, acceptAnyBloodType } = requirement;
  ...
  const buildPayload = (target) => ({
    ...
    data: {
      ...
      bloodType,
      createdBy:         createdBy || '',
      acceptAnyBloodType: acceptAnyBloodType ? 'true' : 'false',
    },
    android: {
      priority: 'high',
      notification: {
  ...
  // ── Path A: Topic push ──────────────────────────────────────────────────────
  // When acceptAnyBloodType is true, send to the general 'bloodconnect_all'
  // topic so all subscribers are notified. Otherwise target the blood-type topic.
  const topic = acceptAnyBloodType
    ? 'bloodconnect_all'
    : 'blood_' + bloodType.replaceAll('+', '_pos').replaceAll('-', '_neg');

  let topicPushSucceeded = false;
  try {
    await firebaseAdmin.messaging().send(buildPayload({ topic }));
    console.log(`🔔 FCM topic push → "${topic}" for ${bloodType} requirement (acceptAny=${!!acceptAnyBloodType})`);
    topicPushSucceeded = true;
  } catch(err) {
  ...
  // ── Path B: Per-device push (fallback ONLY when topic push failed) ──────────
  // Skipped when topic push succeeded — devices subscribed to the topic would
  // otherwise receive a duplicate notification (once via topic, once via token).
  // Only runs if the topic push threw an error, e.g. no devices have subscribed
  // to this topic yet (common on first-ever use before any app installs).
  if (topicPushSucceeded) {
    console.log(`[FCM] Topic push succeeded — skipping per-device push to avoid duplicates.`);
    return;
  }
  ...
  try {
    ...
    if (!acceptAnyBloodType) {
      deviceQuery.bloodType = bloodType;
    }
    const matchingUsers = await User.find(deviceQuery, 'username fcmTokens').lean();
...
async function createInAppNotifications(requirement) {
  try {
    const { bloodType, patientName, hospital, location, unitsRequired, urgency, _id, createdBy, acceptAnyBloodType } = requirement;

    // If acceptAnyBloodType is true, notify ALL available users (not just matching blood type).
    ...
    if (!acceptAnyBloodType) {
      userQuery.bloodType = bloodType;
    }

    const matchingUsers = await User.find(userQuery, 'username').lean();
...
const bloodRequirementSchema = new mongoose.Schema({
  ...
  acceptAnyBloodType:{ type: Boolean, default: false },
...
app.post('/api/auth/otp/verify', async (req, res) => {
  try {
    ...
    const mob = mobile.trim();
    // Dev OTP bypass — accept fixed OTP without checking otpStore
    if (!isDevOtp(otp.trim())) {
      ...
      console.log(`🧪 Dev OTP bypass verified for ${mob}`);
    }
...
app.post('/api/auth/otp/login', async (req, res) => {
  try {
    ...
    if (!isDevOtp(otp.trim())) {
      ...
      console.log(`🧪 Dev OTP bypass login for ${mob}`);
    }
...
// ── OTP: Register new TN User with full donor details ──
app.post('/api/auth/otp/register', async (req, res) => {
  try {
    ...
    if (!isDevOtp(otp.trim())) {
      ...
      console.log(`🧪 Dev OTP bypass register for ${mob}`);
    }
    ...
    console.log(`✅ New TN user registered via OTP → mobile: ${mob}, username: ${autoUsername}`);
...
app.post('/api/auth/register-direct', async (req, res) => {
  try {
    ...
    if (!isDevOtp(otp.trim())) {
      ...
      console.log(`🧪 Dev OTP bypass register-direct for ${mob}`);
    }
    ...
    console.log(`✅ TN user registered → username: ${uname}, mobile: ${mob}`);
...
app.post('/api/auth/mobile/update', authenticate, async (req, res) => {
  try {
    ...
    if (!isDevOtp(otp.trim())) {
      ...
      console.log(`🧪 Dev OTP bypass mobile/update for ${mob}`);
    }
...
// ─── REGISTER (TN user self-registration) ─────────────────
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    console.log(`✅ New TN user registered → username: ${newUser.username}`);
...
// ─── SET AVAILABILITY (lightweight — TN user only) ────────
  ...
  try {
    ...
    for (let i = 0; i < donors.length; i++) {
      ...
      try {
        ...
        const uname  = raw.firstName.toString().trim().toLowerCase().replace(/\s+/g,'') + phone.replace(/\D/g,'').slice(-4);
...
app.post('/api/requirements/:id/donate', authenticate, async (req, res) => {
  try {
    ...
    if (userBT && req_.bloodType !== userBT && !req_.acceptAnyBloodType) {
      return res.status(400).json({ success: false, error: 'Blood type mismatch. Requirement needs ' + req_.bloodType + ', your type is ' + userBT + '.' });
    }
...
// Receives a file from the Flutter app and forwards it to Cloudinary.
// Credentials come from Render environment variables — never from the app.
//
// Render → Environment → Add:
//   CLOUDINARY_CLOUD_NAME    — e.g. dywlppbqa
//   CLOUDINARY_UPLOAD_PRESET — e.g. tnblood  (must be Unsigned in Cloudinary dashboard)
//
...
try {
  ...
  console.warn('⚠️  multer not found — run: npm install multer');
}

app.post('/api/support/upload', async (req, res) => {
  try {
    if (!multer) {
      return res.status(500).json({ success: false, error: 'Server upload module not available. Run: npm install multer' });
    }
    ...
    if (!cloudName || !uploadPreset) {
      ...
      return res.status(500).json({ success: false, error: 'File upload is not configured on the server.' });
    }

    // Parse the incoming multipart file into memory (max 10 MB)
    const uploader = multer({ storage: multer.memoryStorage(), limits: { fileSize: 10 * 1024 * 1024 } }).single('file');
    ...
    // Build multipart/form-data body manually using Node built-ins only.
    // This avoids the 'form-data' npm package — uses only Buffer and crypto.
    const boundary = '----CloudinaryBoundary' + Date.now().toString(16);
    const CRLF = '\r\n';

    const partHeader = Buffer.from(
      '--' + boundary + CRLF +
      'Content-Disposition: form-data; name="file"; filename="' + req.file.originalname + '"' + CRLF +
      'Content-Type: ' + req.file.mimetype + CRLF +
      CRLF
    );
    const partFooter = Buffer.from(CRLF);
    ...
    const body = Buffer.concat([partHeader, req.file.buffer, partFooter, presetPart, closingBoundary]);

    // Use the fetch already available in Node 18+ (no extra package needed)
    const fetchFn = typeof fetch !== 'undefined' ? fetch : require('node-fetch');
    const cloudinaryRes = await fetchFn(
    ...
    if (!cloudinaryRes.ok) {
      console.error('[Upload] Cloudinary rejected upload:', JSON.stringify(data));
      return res.status(502).json({ success: false, error: 'File upload to storage failed.' });
    }
    ...
    res.status(500).json({ success: false, error: 'File upload failed. Please try again.' });
  }
});
...
app.use((err, req, res, next) => {
  // EPIPE / ECONNRESET: client disconnected before response finished — nothing to send back.
  if (err.code === 'EPIPE' || err.code === 'ECONNRESET') return;
  console.error('Unhandled error:', err.message || err);
  // Guard against double-response (route already sent headers before error was thrown)
...
// Suppress EPIPE / ECONNRESET at the process level.
// These fire when a browser navigates away mid-response (bulk upload, export, etc.)
// They are NOT real errors — swallow them silently instead of crashing.
